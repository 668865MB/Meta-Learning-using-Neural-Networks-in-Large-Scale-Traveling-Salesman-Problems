...
def Greedy_algorithm_BN(distance_matrix):
    sys.setrecursionlimit(5000000)
    n = distance_matrix.shape[0]
    Ecost_quantile = np.percentile(distance_matrix[np.nonzero(distance_matrix)], 1)
...
def has_path(edges, x, y):
    ...
    def dfs(node, prev_node):
        ...
        for neigh in graph[node]:
            if neigh != prev_node:  # Check if the neighbor is not the previous node
                if neigh not in visited:
                    if dfs(neigh, node):  # Pass the current node as the previous node
                        return True
        return False